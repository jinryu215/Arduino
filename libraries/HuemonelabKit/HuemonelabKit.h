...
public:
  ...
  int read();                                      // 터치센서 값 읽기
  int count(int duration = 30, int maxCount = 50); // 터치 센서 값 카운트세기(`duration`: 카운트 세는 간격, `maxCount`: 최대 카운트 수 )
...
public:
  ...
  int count(int duration = 150); // 소리감지 카운트세기(`duration`: 카운트 세는 간격)
...
public:
  ...
  int read(int from = 0, int to = 1023); // 빛감지센서값 읽기(analog)
...
public:
  ...
  float read(int from = 0, int to = 1023); // read during sampling time
...
public:
  Buzzer(uint8_t pin);
  void note(char key, unsigned long duration = 1000);
  void play(int number = 1, int tempo = 180);
  void stop();
private:
  uint8_t _pin;
};
